...
if __name__ == '__main__':
    ...
    amp = {'ERN': 0.15, 'P300': 0.2, 'MI4C': 0.3}
    ...
    # path build
    log_path = '/mnt/data1/ljh/results/log/sn_new/'
    if args.muti_label:
        log_path += 'all_target/'
    else:
    ...
    log_name = log_path + (f'baseline{args.amp}.log' if args.baseline else f'sn{args.amp}.log')

    npz_path = '/mnt/data1/ljh/result1214/npz/sn_new/'
    if args.muti_label:
        npz_path += 'all_target/'
    else:
    ...
    npz_name = npz_path + (f'baseline{args.amp}.log' if args.baseline else f'sn{args.amp}.npz')
        ...
        for s in range(1, subject_number):
            ...
            if args.muti_label:
                for w in range(n_class):
                    ...
                    _, x_p, y_p = load(args.dataset, s_id[0],noise_type='sn', clean=False, downsample=False,muti_label=args.muti_label, w=w,sn_amp=args.amp,process=args.process)
            ...
            for target_label in range(n_class):
                ...
                _, x_p, y_original = load(args.dataset, s_id[s], clean=False, downsample=False,noise_type='sn',
